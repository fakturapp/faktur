    ...
    <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-center gap-2 mb-6">
      {steps.map((label, i) => (
        <div key={label} className="flex items-center gap-2">
          {i > 0 && <div className={`h-px w-6 sm:w-10 transition-colors duration-500 ${i <= current ? 'bg-indigo-500' : 'bg-white/10'}`} />}
          <div className="flex items-center gap-1.5">
            <div className={`flex h-6 w-6 items-center justify-center rounded-full text-[10px] font-bold transition-all duration-500 ${
              i < current ? 'bg-emerald-500/20 text-emerald-400' : i === current ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/30' : 'bg-white/5 text-white/30'
            }`}>
              {i < current ? <IconCheck className="h-3 w-3" /> : i + 1}
            </div>
            <span className={`text-[11px] font-medium hidden sm:inline transition-colors duration-500 ${i === current ? 'text-white' : 'text-white/30'}`}>{label}</span>
          </div>
        </div>
      ))}
    </motion.div>
...
export default function CheckoutPayPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  function DlBtn({ className = '' }: { className?: string }) {
    ...
    return (
      <button onClick={dl} disabled={dling} className={`w-full flex items-center justify-center gap-2.5 h-11 rounded-xl border border-white/[0.06] bg-white/[0.03] text-sm text-white/50 hover:text-white/80 hover:bg-white/[0.06] hover:border-white/[0.1] transition-all disabled:opacity-40 ${className}`}>
        {dling ? <Spinner className="h-4 w-4 text-white/50" /> : <IconDownload className="h-4 w-4" />}
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'loading' && (
          <motion.div key="l" {...slide} className="flex flex-col items-center py-28">
            ...
            <p className="mt-5 text-sm text-white/40 font-medium">Chargement du paiement...</p>
          </motion.div>
        )}

        {/* Error */}
        {step === 'error' && (
          <motion.div key="e" {...slide} className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-2xl p-8 shadow-2xl">
            <div className="flex flex-col items-center text-center">
              ...
              <h2 className="text-lg font-semibold text-white mb-2">Lien introuvable</h2>
        ...
        {/* Expired */}
        {step === 'expired' && (
          <motion.div key="x" {...slide} className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-2xl p-8 shadow-2xl">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-white/50 leading-relaxed">Ce lien de paiement a expiré. Contactez l&apos;émetteur de la facture pour obtenir un nouveau lien.</p>
            </div>
          </motion.div>
        )}
        ...
        {/* Password */}
        {step === 'password' && (
          <motion.div key="p" {...slide} className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-2xl p-8 shadow-2xl">
            <div className="flex flex-col items-center text-center mb-7">
              ...
              <p className="text-sm text-white/50 mt-1.5">Ce lien est protégé. Entrez le mot de passe communiqué par l&apos;émetteur.</p>
            </div>
            <div className="space-y-3">
              <input type="password" value={pw} onChange={(e) => setPw(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && submitPw()} placeholder="Mot de passe" autoFocus
                className="w-full h-12 rounded-xl border border-white/[0.08] bg-white/[0.04] px-4 text-sm text-white placeholder:text-white/25 outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/10 transition-all" />
              ...
              <button onClick={submitPw} disabled={busy || !pw}
                ...
                {busy ? <Spinner className="h-4 w-4" /> : 'Accéder'}
              </button>
            </div>
          </motion.div>
        )}
        ...
        {/* Method */}
        {step === 'method' && d && (
          <motion.div key="m" {...slide} className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-2xl p-8 shadow-2xl">
            <div className="text-center mb-8">
              {d.companyName && <p className="text-[11px] font-semibold text-white/30 uppercase tracking-[0.15em] mb-3">{d.companyName}</p>}
              <motion.p initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.1, type: 'spring', bounce: 0.3 }}
                className="text-4xl font-bold text-white tracking-tight">{amt}</motion.p>
              <p className="text-sm text-white/40 mt-2">Facture {d.invoiceNumber}</p>
            </div>

            <p className="text-[10px] font-bold text-white/25 uppercase tracking-[0.2em] mb-3">Mode de paiement</p>

            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }} onClick={pickBank} disabled={busy}
              className="w-full flex items-center gap-4 p-4 rounded-xl border border-white/[0.06] bg-white/[0.03] hover:bg-indigo-500/5 hover:border-indigo-500/20 transition-all text-left group mb-2.5">
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-white/35 mt-0.5">Coordonnées bancaires fournies</p>
              </div>
              {busy ? <Spinner className="h-4 w-4 text-indigo-400" /> : (
            ...
            <div className="flex items-center gap-4 p-4 rounded-xl border border-white/[0.04] opacity-25 cursor-not-allowed mb-6">
              <div className="h-11 w-11 rounded-xl bg-white/[0.04] flex items-center justify-center shrink-0">
                <svg className="h-5 w-5 text-white/30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><rect x="1" y="4" width="22" height="16" rx="2"/><line x1="1" y1="10" x2="23" y2="10"/></svg>
              </div>
              <div className="flex-1"><p className="text-sm text-white/40">Carte bancaire</p><p className="text-[11px] text-white/20">Bientôt disponible</p></div>
              <IconLock className="h-3.5 w-3.5 text-white/15" />
            </div>
        ...
        {/* IBAN */}
        {step === 'iban' && d && (
          <motion.div key="i" {...slide} className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-2xl p-8 shadow-2xl">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-white/40 mt-0.5">Facture {d.invoiceNumber}</p>
            </div>
            ...
            {ib?.iban ? (
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="space-y-4">
                <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <span className="text-[10px] font-bold text-white/25 uppercase tracking-[0.2em]">Coordonnées bancaires</span>
                  </div>
                  {ib.bankName && <div className="mb-4"><p className="text-[10px] text-white/20 uppercase tracking-wider mb-1">Banque</p><p className="text-sm font-medium text-white">{ib.bankName}</p></div>}
                  <div className="mb-4">
                    <p className="text-[10px] text-white/20 uppercase tracking-wider mb-1">IBAN</p>
                    <div className="flex items-center gap-2">
                      <p className="text-[15px] font-mono font-semibold text-white tracking-[0.15em] break-all leading-relaxed">{ib.iban}</p>
                      <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={cp}
                        className="shrink-0 h-8 w-8 rounded-lg flex items-center justify-center text-white/30 hover:text-white hover:bg-white/[0.06] transition-all" title="Copier">
                        {copied ? <IconCheck className="h-4 w-4 text-emerald-400" /> : <IconCopy className="h-4 w-4" />}
                      </motion.button>
                    </div>
                  </div>
                  {ib.bic && <div><p className="text-[10px] text-white/20 uppercase tracking-wider mb-1">BIC</p><p className="text-sm font-mono font-medium text-white tracking-wider">{ib.bic}</p></div>}
                </div>
              ...
              <div className="rounded-xl bg-white/[0.03] border border-white/[0.06] p-5 text-center">
                <p className="text-sm text-white/40">Contactez l&apos;émetteur pour les coordonnées bancaires.</p>
              </div>
            )}

            <div className="flex gap-3 mt-6">
              <motion.button whileTap={{ scale: 0.97 }} onClick={() => setStep('method')}
                className="flex-1 h-11 rounded-xl border border-white/[0.06] text-sm font-medium text-white/50 hover:text-white hover:border-white/[0.1] transition-all flex items-center justify-center gap-2">
        ...
        {/* Confirm */}
        {step === 'confirm' && (
          <motion.div key="c" {...slide} className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-2xl p-8 shadow-2xl">
            ...
            <div className="flex gap-3">
              <motion.button whileTap={{ scale: 0.97 }} onClick={() => setStep('iban')}
                className="flex-1 h-11 rounded-xl border border-white/[0.06] text-sm font-medium text-white/50 hover:text-white transition-all">Annuler</motion.button>
              <motion.button whileTap={{ scale: 0.97 }} onClick={pay} disabled={busy}
                ...
                {busy ? <Spinner className="h-4 w-4" /> : 'Confirmer'}
              </motion.button>
            </div>
          </motion.div>
        )}
        ...
        {/* Pending */}
        {step === 'pending' && d && (
          <motion.div key="w" {...slide} className="rounded-2xl border border-white/[0.06] bg-white/[0.03] backdrop-blur-2xl p-8 shadow-2xl">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-white/50 leading-relaxed mb-2">
                Facture <strong className="text-white">{d.invoiceNumber}</strong> — le paiement a été signalé et est en attente de confirmation par l&apos;émetteur.
              </p>
              {d.maskedEmail && (
                <div className="flex items-center gap-2 text-white/30 mb-6">
                  <IconMail className="h-3.5 w-3.5" />
                  <p className="text-xs">Vous serez informé par mail à <span className="text-white/50 font-mono">{d.maskedEmail}</span></p>
                </div>
              )}
              <DlBtn />
            </div>
        ...
        {step === 'done' && d && (
          <motion.div key="d" {...slide} className="rounded-2xl border border-emerald-500/10 bg-white/[0.03] backdrop-blur-2xl p-8 shadow-2xl">
            <div className="flex flex-col items-center text-center">
              ...
              <p className="text-sm text-white/50 leading-relaxed mb-2">
                Le paiement de la facture <strong className="text-white">{d.invoiceNumber}</strong> a été confirmé par l&apos;émetteur. Tout est en ordre.
              </p>
              {d.maskedEmail && (
                <div className="flex items-center gap-2 text-white/30">
                  <IconMail className="h-3.5 w-3.5" />
                  <p className="text-xs">Une confirmation a été envoyée à <span className="text-white/50 font-mono">{d.maskedEmail}</span></p>
                </div>
              )}
            </div>
          </motion.div>
